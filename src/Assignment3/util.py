...
def first_row(df):
    ...
    df_select=df.withColumn("row",row_number().over(window1)).filter(col("row")==1).drop("row")
    return df_select

def highest_salary(df):
    window1=Window.partitionBy("department").orderBy(col("salary").desc())
    df_highsal=df.withColumn("row",row_number().over(window1)).filter(col("row")==1).drop("row")
    return df_highsal

def totalsal_avg_high_low(df):
    window2=Window.partitionBy("department").orderBy("salary")
    real_data=Window.partitionBy("department")
    df_salary=df.withColumn("row",row_number().over(window2))\
